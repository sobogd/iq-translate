...
**General** — `llama-server` with one model, four slots, 16384 tokens of
...
llama-server -m ~/models/Qwen3.5-4B-UD-Q4_K_XL.gguf --alias qwen/qwen3.5-9b \
  --host 127.0.0.1 --port 1234 --jinja -ngl 999 -c 16384 --flash-attn on \
...
| `LLM_CTX` | `16384` | `16384` | must equal the general server's `-c` |
